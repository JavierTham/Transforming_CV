...
def encode_labels(target, labels):
...
def split(df, *, X, y, train_size=None, test_size=None, group_id, random_state=42):
	gss = GroupShuffleSplit(n_splits=1, train_size=train_size, test_size=test_size, random_state=random_state)
	train_idx, test_idx = next(gss.split(df.loc[:, X], df.loc[:, y], groups=df.loc[:, group_id]))
	return df.iloc[train_idx, :], df.iloc[test_idx, :]

if __name__ == "__main__":
	df = create_df(VIDEO_PATH, LABELS)

	encoded_labels = encode_labels(df.loc[:, "action"], LABELS)

	groups = df.loc[:, "group_id"].unique()
	encoded_groups = encode_labels(df.loc[:, "group_id"], groups)
	
	df.loc[:, "action"] = encoded_labels
	df.loc[:, "group_id"] = encoded_groups
	# df.to_csv("../data/UCF_df.csv", index=False)

	train_df, test_df = split(df, X="idx", y="action", group_id="group_id", train_size=0.7)
	val_df, test_df = split(test_df, X="idx", y="action", group_id="group_id", test_size=0.5)
